...
# WHERE species IN ('BET') AND country IN ('747') AND year > 2000 AND year < 2003
# ;"
...
# 
# res_dimensions_and_variables = st_read(con, query = query)
# 
# 
# nc_file <- TunaAtlas_database_to_NetCDF(df=res_dimensions_and_variables)
# 
# # Load raster and set projection and lat/lon extent
# pr <- raster(nc_file, varname="v_catch")
# 
...
# st_area(st_as_sf(res_dimensions_and_variables,wkt=geom_wkt))
...
#Specify the name of the dimensions to be taken into account
...
######################################################################
##### WRITE DEFINITION OF SPATIAL and TEMPORAL DIMENSIONS  ##########
######################################################################
# XYT dimensions
...
####################################################################################################
##### Add Metadata in NetCDF header: write the attributes of "dimensions variables"  ##########
####################################################################################################
...
##### Write GLOBAL ATTRIBUTES  in NetCDF header ##########
#################################
