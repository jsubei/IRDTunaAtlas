 SELECT sub_qry3.year,
   ...
   FROM
	...
	LEFT JOIN area.area_labels USING (id_area)
          ...
          WHERE "time".year::numeric <= 2005::numeric AND metadata.identifier = 'global_catch_1deg_1m_ps_bb_firms_level0'::text AND metadata.id_metadata = tab.id_metadata
     ...
     JOIN ( SELECT 
	    ...
	    subsub.id_area
           ...
           FROM (SELECT sub_qry.reference_year,
	 	     ...
	 	     area_labels.id_area
	           ...
	           FROM metadata.metadata, fact_tables.catch tab  
                  ...
                  WHERE metadata.identifier = 'global_catch_1deg_1m_ps_bb_firms_level0'::text AND metadata.id_metadata = tab.id_metadata
          ...
          GROUP BY subsub.reference_year, subsub.month, subsub.c_esp, subsub.c_g_engin,subsub.id_area) sub_qry2 
          
          USING (year, month, c_esp, c_g_engin,id_area) 
