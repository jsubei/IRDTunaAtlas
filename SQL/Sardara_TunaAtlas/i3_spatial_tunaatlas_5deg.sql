 SELECT sub_qry3.year,
   ...
   FROM
	...
	LEFT JOIN area.area_labels USING (id_area)
          ...
          WHERE "time".year::numeric <= 2005::numeric AND metadata.identifier = 'global_catch_5deg_1m_firms_level0'::text AND metadata.id_metadata = tab.id_metadata
           ...
           FROM (SELECT sub_qry.reference_year,
	 	     ...
	 	     area_labels.id_area
	           ...
	           FROM metadata.metadata, fact_tables.catch tab  
                  ...
                  WHERE metadata.identifier = 'global_catch_5deg_1m_firms_level0'::text AND metadata.id_metadata = tab.id_metadata
